...
parser.add_argument('--lsp-root', default='./data/LSP', metavar='DIR',
...
parser.add_argument('--ckpt-dir', default='./checkpoints', metavar='DIR',
...
def train():
    ...
    model = cpm_model.CPM(train_data.num_keypoints).to(device)
    model.train()
    if args.cuda:
        model.cuda()
    lr = args.lr
        ...
        for batch_idx, sample in enumerate(train_loader):
            ...
            if global_step % args.print_freq == 0:
                try:
                    ...
                    writer.add_scalar('MSELoss', loss_log.item(), global_step=global_step)
                    writer.add_scalar('Weighted MSELoss', loss.item(), global_step=global_step)
                    end_time = time.time()
                    last_count = count
                except NameError:
                    end_time = time.time()
